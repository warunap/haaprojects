 ...
 * 
 * $Id: tibjmsExtensions.java 21731 2006-05-01 21:41:34Z $
 * 
 */
...
public class tibjmsExtensions implements Runnable
{
    ...
    TopicPublisher      checkPublisher;
    TopicSubscriber     checkSubscriber;
        ...
        public void onMessage(Message message)
        {
            ...
            try
            {
                ...
                if (receivedCount == count) {
                    checkPublisher.publish(message);
                }
            }
    ...
    public void doRun(int deliveryMode, int acknowledgeMode, boolean warmupMode) throws JMSException
    {
        ...
        publishSession   = connection.createTopicSession(false,acknowledgeMode);
        subscribeSession = connection.createTopicSession(false,acknowledgeMode);

        /* little thread printing dots on the screen while
         * nothing else happens
         */
        Thread tracker = null;

        /* if not yet created, create main topic and the cjeck topic used
        ...
        subscriber = subscribeSession.createSubscriber(topic);
        subscriber.setMessageListener(new MyMessageListener(acknowledgeMode,warmupMode));

        /* create publisher and subscriber to deal with the
         * final confirmation message
         */
        checkPublisher  = subscribeSession.createPublisher(checkTopic);
        checkSubscriber = publishSession.createSubscriber(checkTopic);
        ...
        /*
         * and wait for the subscriber to receive all messages
